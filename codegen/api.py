...
def ballistics_api(
    *,
    ...
    lines = [
        ...
        *_optimize_launch_direction_api(weapon_parameters, prefix=indent, indent=indent),
    ...
    for direction_overload in _DIRECTION_OVERLOADS:
        for velocity_overload in _VELOCITY_OVERLOADS:
            lines += [
                ...
                *(f"{indent}{p}," for p in [velocity_overload.parameter] if p),
                *_model_parameter_decls(weapon_parameters, indent, ","),
                f"{__}const scalar start_time,",
                f"{__}const scalar end_time,",
    ...
    for velocity_overload in _VELOCITY_OVERLOADS:
        lines += [
            ...
            *_model_parameter_decls(weapon_parameters, indent, ","),
            ...
            f"{__}const uint32_t max_launch_direction_optimizer_iterations = 25) const",
            f"{{",
            ...
            f"{__}{__}{{{', '.join(_model_parameter_args(weapon_parameters))}}},",
            f"{__}{__}time_of_flight,",
            f"{__}{__}max_launch_direction_optimizer_iterations);",
            f"}}",
        ]
    return [prefix + line for line in lines]
    ...
    for velocity_overload in _VELOCITY_OVERLOADS:
        lines += [
            ...
            *_model_parameter_decls(weapon_parameters, indent, ","),
            ...
            f"{__}{__}{__}max_launch_direction_optimizer_iterations));",
            f"}}",
        ]
    return [prefix + line for line in lines]
    ...
    for velocity_overload in _VELOCITY_OVERLOADS:
        lines += [
            ...
            *_model_parameter_decls(weapon_parameters, indent, ","),
            ...
            f"{__}const scalar miss_distance_threshold = 1.0,",
            f"{__}const scalar segment_size = 0.5,",
            f"{__}const uint32_t max_time_of_flight_optimizer_iterations = 30,",
            f"{__}const uint32_t max_launch_direction_optimizer_iterations = 25) const",
            f"{{",
            ...
            f"{__}{__}miss_distance_threshold,",
            f"{__}{__}segment_size,",
            f"{__}{__}max_time_of_flight_optimizer_iterations,",
            f"{__}{__}max_launch_direction_optimizer_iterations);",
            f"}}",
        ]
    return [prefix + line for line in lines]
